...
**Чёрное превью на месте видео (4K/HDR — или *всё* видео на некоторых ТВ):**
Если в превью Hyperion/WLED/HyperHDR на месте видео — чёрный прямоугольник, хотя меню, превью-миниатюры, фото и тестовые цвета из приложения захватываются нормально — это **не DRM**. На Android видео рисуется на отдельной **аппаратной видео-плоскости (overlay)**, которую собирает дисплейное железо в обход SurfaceFlinger; `MediaProjection` видит только то, что собрал SurfaceFlinger, поэтому область видео приходит сплошным чёрным. Обычно это касается только 4K / HEVC 10-bit / HDR10 / Dolby Vision, а обычный 1080p идёт через штатный стек — **но некоторые ТВ (например, ряд моделей Sony) гонят через overlay _всё_ видео**, поэтому чернеет даже 1080p и локальные файлы с флешки (чёрный *локальный* файл исключает DRM). Это ограничение платформы — оно затрагивает любые ambient-light приложения на Android (Hyperion Android, Lightpack apps и т.д.), фикса для штатного пути `MediaProjection` нет.

Обходные пути:
1. **Понизить разрешение воспроизведения до 1080p** в плеере/сервере (Jellyfin → выставить max bitrate / resolution для устройства; Kodi → настройки воспроизведения; mpv → `--hwdec=no` + `--vo=gpu`). 1080p почти всегда идёт через обычную композицию.
2. **Выключить аппаратное декодирование** в плеере (Jellyfin Android: Настройки → Плеер → отключить "Prefer FMP4"; в mpv → `--hwdec=no`). Программное декодирование пускает кадр через композицию, захват возвращается.
3. **Для устройств с root** — попробовать метод захвата `Screencap (Root)` или `Screencap (Shell)` в настройках. Они идут через бинарь `screencap` вместо `MediaProjection` и на многих устройствах обходят overlay-флаг.
4. **На ТВ с SoC от MediaTek** — экспериментальный метод `MTK THAL Capture` захватывает прямо из вендорского DIP-движка (до композиции), и 4K HDR там работает. Требует root и MTK-чип.
